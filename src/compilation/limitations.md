...
```c
void scale_add(float *a, float *b, float *c, int n) {
    for (int i = 0; i < n; i++)
        a[i] = b[i] * s + c[i];  // Can 'a' overlap 'b' or 'c'?
}
```
...
float sum = 0;
for (int i = 0; i < n; i++)
    sum += a[i];  // Reassociation would require -ffast-math
```
...
```c
__attribute__((const)) int expensive_computation(int x);
```
...
void copy(char *dst, char *src, int n) {
    for (int i = 0; i < n; i++)
        dst[i] = src[i];  // Could src and dst overlap with a gap?
}
```
...
```c
double foo(double x) { return bar(x); }  // Cannot inline, bar is in another TU
```
...
```c
void memset_zero(char *buf, int n) {
    for (int i = 0; i < n; i++)
        buf[i] = 0;
}
// buf is never read afterward — the store might be dead
```
